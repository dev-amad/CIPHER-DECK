...
export default function SyntaxFixPuzzle({ puzzle, sector, onSolve, onBack }) {
    ...
    const handleDeploy = () => {
        ...
        const targetOpt = puzzle.options.find((o) => o.id === choice);

        setTimeout(() => {
            setBusy(false);

            if (targetOpt.correct) {
                sound.playSuccess();
                setFixed(true);
                setOutputLogs([
                    `[PATCH DEPLOY] Applying patch candidate: "${targetOpt.label}"`,
                ...
                setOutputLogs([
                    `[PATCH DEPLOY] Applying patch candidate: "${targetOpt.label}"`,
                    `[COMPILER REJECT] Fatal syntax error: ${targetOpt.explanation}`,
            ...
            <div className="bg-[#0c0f14] border-2 border-amber-500/40 rounded-xl overflow-hidden shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]">
                ...
                <div className="p-4 font-mono text-xs sm:text-sm overflow-x-auto">
                    {puzzle.codeLines.map((item) => {
                        const isErr = item.line === puzzle.errorLine;
                        const matchOpt = puzzle.options.find((o) => o.id === choice);

                        return (
                            <div
                                key={item.line}
                                className={`flex items-center space-x-4 py-0.5 px-2 rounded ${isErr
                                        ? fixed
                            ...
                            >
                                ...
                                <span className="flex-1 whitespace-pre">
                                    {isErr && fixed && matchOpt
                                        ? item.text.replace(puzzle.codeLines.find(l => l.line === puzzle.errorLine).text.trim(), matchOpt.label)
                                        : item.text}
                                </span>
                                {isErr && (
                                    <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded select-none ${fixed ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300 animate-pulse'
                                        }`}>
                                        {fixed ? '[PATCHED]' : '[SYNTAX_ERR]'}
                                    </span>
                                )}
                            </div>
                        );
            ...
            <div className="bg-[#121417] border border-amber-500/40 rounded-xl p-4 sm:p-5 space-y-4">
                    ...
                    {puzzle.options.map((opt) => {
                        ...
                        return (
                            <div
                                ...
                                className={`p-3 rounded-lg border text-xs cursor-pointer transition-all duration-150 select-none ${active
                            ...
                            >
                                <div className="flex items-center space-x-2 mb-1.5">
                                    <span className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center text-[9px] font-bold ${active ? 'border-amber-400 bg-amber-400 text-neutral-950' : 'border-neutral-600 text-neutral-400'
                                        }`}>
                                        {active ? '✓' : ''}
                                    </span>
                ...
                <div className="pt-3 border-t border-neutral-800 flex justify-end">
                    <button
                        ...
                        className={`px-6 py-2.5 rounded font-bold tracking-wider text-xs sm:text-sm transition-all cursor-pointer ${!choice || busy
                                ? 'bg-neutral-800 text-neutral-500 cursor-not-allowed'
                                : 'bg-amber-500 text-neutral-950 hover:bg-amber-400 shadow-[0_0_15px_rgba(255,176,0,0.5)] active:scale-95'
                            }`}
                    >
                        {busy ? '[ COMPILING PATCH... ]' : 'COMPILE & TEST PATCH >>'}
                    </button>
                </div>
            </div>
        </div>
