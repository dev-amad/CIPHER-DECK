...
export default function LogicGatePuzzle({ puzzle, sector, onSolve, onBack }) {
    ...
    const handleCycle = (slotId) => {
        sound.playToggle();
        const node = puzzle.gateSlots.find((s) => s.id === slotId);
        if (!node) return;

        const activeType = config[slotId];
        const idx = node.allowedTypes.indexOf(activeType);
        const nextIdx = (idx + 1) % node.allowedTypes.length;
        const nextType = node.allowedTypes[nextIdx];

        setConfig((prev) => ({ ...prev, [slotId]: nextType }));
        setResults(null);
    };
    ...
    const runTest = () => {
        ...
        setTimeout(() => {
            const outcome = evaluateCircuit(puzzle, config);
            setResults(outcome);
            setRunning(false);
                ...
                <div className="relative z-10 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {puzzle.gateSlots.map((slot) => {
                            const current = config[slot.id];
                            return (
                                <div
                                    key={slot.id}
                                    onClick={() => handleCycle(slot.id)}
                                ...
                                >
                                    ...
                                    <div className="mt-2.5 flex items-center justify-center space-x-1 text-[9px] text-neutral-400">
                                        {slot.allowedTypes.map((t) => (
                                            <span
                                                key={t}
                                                className={`px-1 py-0.5 rounded ${t === current
                    ...
                    <div className="pt-4 border-t border-neutral-800 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <button
                            onClick={runTest}
                            disabled={running}
                            className={`px-6 py-2.5 rounded font-bold tracking-wider text-xs sm:text-sm transition-all duration-150 cursor-pointer ${running
                                    ? 'bg-neutral-800 text-neutral-500 cursor-wait'
                                    : 'bg-cyan-500 text-neutral-950 hover:bg-cyan-400 shadow-[0_0_15px_rgba(0,232,198,0.5)] active:scale-95'
                                }`}
                        >
                            {running ? '[ PROPAGATING CURRENT... ]' : 'RUN_DIAGNOSTIC >>'}
                        </button>
                    </div>
                </div>
            </div>
            ...
            {results && (
                <div className="bg-[#121417] border border-cyan-500/30 rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between text-xs font-bold">
                        ...
                        <span className={results.allPassed ? 'text-emerald-400' : 'text-red-400'}>
                            {results.allPassed ? '>>> ALL VECTORS PASSED (CIRCUIT STABLE)' : '>>> CIRCUIT DRIFT DETECTED (MISMATCH)'}
                        </span>
                    </div>
